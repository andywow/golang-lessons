...
func Copy(from, to string, limit, offset int) error {
	if offset < 0 {
		return fmt.Errorf("Offset of source file must be greater then or equal to zero")
	}
	if limit < 0 {
		return fmt.Errorf("Limit must be greater then or equal to zero (to read all file)")
	}
